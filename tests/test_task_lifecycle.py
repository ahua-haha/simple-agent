...
@pytest.mark.asyncio
async def test_common_task_lifecycle_exposes_index_tree_tool(tmp_path):
    ...
    result = await tools["index_tree"].execute("call_1", {"path": "src", "entry_limit": 10})

    assert "index_tree" in tools
    assert "index_upsert" not in tools
    assert "app.py" in result.content[0].text
